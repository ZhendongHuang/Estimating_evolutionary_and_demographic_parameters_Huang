...
name1 = "xx_true_list="+str(sam_size)+"_expne0001_len108.dat"
name2 = "nt_true_list="+str(sam_size)+"_expne0001_len108.dat"
...
#         timereg.append(gitruenode[abduse[ind][4]])
#     smoothing = KernelReg(endog= lenlistreg,
#                      exog= timereg, reg_type='lc',
#                      var_type='c', bw='cv_ls' ) 
#     meanlgt = smoothing.fit(x)[0]

    ### estimate tmrca without smoothing
#     tmrcalist1_nos = []
#     for i in range(len(abduse)):
#         tmrcalist1_nos.append(gihatcon[abduse[i][4]] )
 


    meanlgt = [sum( ( numpy.array(abdlen))*(numpy.array(tmrcalist1)<x[1]) ) / sum(numpy.array(tmrcalist1)<x[1] ) ]
    for i in range(1,len(x)-1):
         meanlgt.append(sum( ( numpy.array(abdlen))* \
                            ( x[i-1]<numpy.array(tmrcalist1) )*\
                               (numpy.array(tmrcalist1)<x[i+1]) ) / sum( (x[i-1]<numpy.array(tmrcalist1))*\
                                                                      (numpy.array(tmrcalist1)<x[i+1]) ) )
    ...
    nt = savgol_filter(nt, window_length=10, polyorder=1, mode="nearest")
