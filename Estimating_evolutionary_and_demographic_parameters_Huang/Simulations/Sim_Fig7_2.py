...
#prior = unif(pop_size+-5000 )  unif(-0.00002,0.00002)
...
for iter in range(totaliter):
    ...
    inipop = numpy.random.uniform(5000,15000)
    increrate = numpy.random.uniform(-0.00002,0.00002)
    ## put a population cap = 20000*2
    if increrate <0:
        turn = numpy.log(20000/inipop)/(-increrate)
...
#                 print("finding abd pair propotion ",i/2/sam_size)
    ...
    if abs(simmean - obsmean) <=500 and abs(simdev- obsdev) <=1000:
        acceptlist.append([inipop,increrate])
    
    
run = 1

name = "aaacceptlist"+str(run)+"pop10000_neconst.dat"
