...
###############################################################################
    ...
    mnodes = mts.tables.mutations.node.tolist()
    
    maxnode = mnodes[-1]
    
    numgenotype = numpy.random.poisson( seq_len*2*sam_size*epsilon )
    locgenotype = numpy.random.uniform( 0, seq_len*sam_size , numgenotype)
    mpos = mpos + numpy.floor(locgenotype % seq_len).tolist()
    mnodes = mnodes + numpy.floor(locgenotype/seq_len).tolist()
    mnodes = numpy.array(mnodes)
    ...
    maxnode = len(mts.tables.nodes.flags)
    clocate = []
    curc = -1
                        ...
                        if begin1==end:
                            ...
                            for k in range(begin1+begin2,end):
                                ...
                                if el_sc[k]<=curleft and er_sc[k] > curleft:
                                    ...
                                    break
                                elif el_sc[k]>curleft:
                                    curright = min(el_sc[k],curright)
                                    break
                        if find ==0:
                            break
...
###############################################################################
        ...
        numgenotype = numpy.random.poisson( seq_len*2*sam_size*epssim )
        locgenotype = numpy.random.uniform( 0, seq_len*sam_size , numgenotype)
        mpos = mpos + numpy.floor(locgenotype % seq_len).tolist()
        mnodes = mnodes + numpy.floor(locgenotype/seq_len).tolist()
        mnodes =numpy.array(mnodes)
...
byrun = 1
name1 = "zmusimall_seq_len="+str(seq_len)+"trun"+str(min_span)+"run="+str(byrun) +"_sam_size="+str(sam_size)+"_eps(no)=" + str(epsilon)+"_noconver_constne_ttibd.dat"
name3 = "zsta1simall_seq_len="+str(seq_len)+"trun"+str(min_span)+"run="+str(byrun) +"_sam_size="+str(sam_size)+"_eps(no)=" + str(epsilon)+"_noconver_constne_ttibd.dat"
name5 = "zsta1inf_seq_len="+str(seq_len)+"trun"+str(min_span)+"run="+str(byrun) +"_sam_size="+str(sam_size)+"_eps(no)=" + str(epsilon)+"_noconver_constne_ttibd.dat"
name7 = "zabdlen_seq_len="+str(seq_len)+"trun"+str(min_span)+"run="+str(byrun) +"_sam_size="+str(sam_size)+"_eps(no)=" + str(epsilon)+"_noconver_constne_ttibd.dat"
