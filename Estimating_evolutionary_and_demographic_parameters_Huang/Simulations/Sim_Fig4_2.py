...
name1 = "xx_true_list="+str(sam_size)+"_constne_bottleneck1.dat"
name2 = "nt_true_list="+str(sam_size)+"_constne_bottleneck1.dat"
 


demo_model = msprime.Demography.isolated_model([pop_size1], growth_rate=[0])
demo_model.add_population_parameters_change(40000,  initial_size=pop_size2, growth_rate=0, population=None)
...
#     ftss[0]=1/2/pop_size1

####
####
####
#     meanlgt = [sum( ( numpy.array(abdlen))*(numpy.array(tmrcalist)<x[1]) ) / sum(numpy.array(tmrcalist)<x[1] ) ]
#     for i in range(1,len(x)-1):
#          meanlgt.append(sum( ( numpy.array(abdlen))* \
#                             ( x[i-1]<numpy.array(tmrcalist) )*\
#                                (numpy.array(tmrcalist)<x[i+1]) ) / sum( (x[i-1]<numpy.array(tmrcalist))*\
#                                                                       (numpy.array(tmrcalist)<x[i+1]) ) )
#     meanlgt.append(meanlgt[-1])   
#     meanlgt = savgol_filter(meanlgt, window_length=3, polyorder=1, mode="nearest")    
#     meanl = statistics.mean(abdlen)
    ...
    ftss = savgol_filter(ftss, window_length=20, polyorder=1, mode="nearest")  
...
#     fts = fts/( sum(fts)*(x[1]-x[0]) )
    ...
    nt = savgol_filter(nt, window_length=20, polyorder=1, mode="nearest")
    ...
    plt.plot(xx,nt)
    plt.ylim(0,80000)
    plt.plot([0,40000],[40000,40000])
    plt.plot([40000,150000],[10000,10000])
    plt.xlim(0,90000)    
    print(iter)
